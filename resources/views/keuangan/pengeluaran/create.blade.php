    ...
    <x-card>
        ...
        <form action="{{ route('keuangan.store') }}" method="POST" class="space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div><label class="block text-sm font-semibold mb-1">Kategori *</label><input type="text" list="catList" name="kategori" value="{{ old('kategori') }}" placeholder="Konsumsi, Perbaikan..." class="w-full px-3 py-2 border rounded-md text-sm" required><datalist id="catList">@foreach($categories as $c)<option value="{{ $c }}">@endforeach</datalist></div>
                ...
                <div><label class="block text-sm font-semibold mb-1">Sumber Dana</label><input type="text" name="sumber_dana" value="{{ old('sumber_dana') }}" placeholder="Kas RT" class="w-full px-3 py-2 border rounded-md text-sm"></div>
            ...
            <div class="flex justify-end gap-2 pt-4"><a href="{{ route('keuangan.pengeluaran.index') }}" class="px-4 py-2 border rounded-md text-sm">Batal</a><button type="submit" class="px-6 py-2 bg-red-600 text-white rounded-md text-sm font-semibold">Simpan</button></div>
        </form>
    </x-card>
</div>
@endsection
